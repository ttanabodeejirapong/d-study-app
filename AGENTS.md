...
Rules:
1. Every subject must have its own explicit storage namespace for academic state, quiz drafts, notes, history, flashcard progress/sessions, recovery snapshots, and any future subject-specific data.
...
5. Shared settings may use a shared preferences key, but only appearance/preferences belong there. Progress, pages, notes, note images, quiz history, confidence data, drafts, mastery state, flashcard progress/sessions, and recovery data remain subject-isolated.
